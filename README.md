...
=============
 ...
 This software is a plugin enabling CCS compiler and linker support for the
 Mindc toolchain (http://mind.ow2.org). It enables the use of CCS-specific
...
- 	Une classe CCSCompilerWrapper qui étend le GCCCompilerWrapper et qui doit
	être complétée pour adapter la ligne de commande générée pour exécuter le
	préprocesseur, le compilateur et le linker.
- 	Une classe CCSCompilerModule. La toolchain MIND utilise un outil
	d’injection de dépendance nommé ‘Google Guice’. Cette classe
	CCSCompilerModule est un module Guice qui spécifie que l’interface
	« CompilerWrapper » est implémentée par la class « CCSCompilerWrapper ».
...
- 	Un fichier ‘mind-plugin.xml’. C’est le descripteur de l’extension. Le
	...
	o   La première ajoute le flag ‘—ccs‘ sur la ligne de commande ‘mindc’.
		Avec cette extension, ‘mindc’ peut reconnaitre le flag ‘’ccs’ sur la
		ligne de commande.
	o   La deuxième ajoute le module Guice « CCSCompilerModule ». Ce module
		...
		active que si le flag ‘—ccs’ est présent sur la ligne de commande de
		mindc. Ceci permet d’utiliser le CCSCompilerWrapper si et seulement si
		le flag ‘-ccs’ est spécifié sur la ligne de commande de mindc.
...
il faut lancer mindc avec l’option ‘—ccs’.